 ...
 * @see https://github.com/FunkinCrew/Funkin/blob/main/source/funkin/util/ReflectUtil.hx
 */
...
class ReflectUtil
{
	...
	public static function resolveClass(name:String):Null<Class<Any>>
	{
		...
		if (resolved == null)
			return null;
		if (!Std.isOfType(resolved, Class))
			return null;
	...
	public static function getClassNameOf(obj:Any):String
	{
		if (obj == null)
			return 'Unknown';
		if (obj is String && Type.resolveClass(obj) != null)
			return obj;
		@:nullSafety(Off)
		var cls = Type.getClass(obj);
		if (cls == null)
	...
	{
		if (pathResolveCache.exists(path))
			return pathResolveCache.get(path);
		...
		else
		{
			...
			if (resultEnm != null)
				pathResolveCache.set(path, resultEnm);
			return resultEnm;
		}
	}
	...
	static function isAccessAllowed(obj:Dynamic, ?varName:String):Bool
	{
		...
		if (varName != null)
		{
			...
			if (blacklistedFields.contains(varName))
				return false;
		}

		if (PolymodScriptClass.importOverrides.exists(key) && PolymodScriptClass.importOverrides.get(key) == null)
			return false;
